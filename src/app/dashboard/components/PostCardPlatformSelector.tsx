...
interface PostCardPlatformSelectorProps {
  postId: string;
  platforms?: Platform[];
  onPlatformToggle?: (postId: string, platformId: string) => void;
}
...
export default function PostCardPlatformSelector({ 
  postId,
  platforms = defaultPlatforms, 
  onPlatformToggle
}: PostCardPlatformSelectorProps) {
  return (
    <div className="mt-2 flex flex-wrap gap-1">
      {platforms.map((platform) => (
        <button
          key={`${postId}-${platform.id}`}
          onClick={(e) => {
            e.stopPropagation();
            if (onPlatformToggle) {
              onPlatformToggle(postId, platform.id);
            }
          }}
          ...
          title={`${platform.selected ? 'Remove from' : 'Add to'} ${platform.name}`}
        >
          {platform.icon}
        </button>
      ))}
